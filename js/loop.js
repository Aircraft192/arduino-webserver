            ...
            if(json_config['D' + i].mode == "0"){
                if(json_data.target['D' + i] == -1){
                    ...
                    if (document.getElementById("button_duration_d" + i) == null) {
                        document.getElementById("table_cell_press_d" + i).innerHTML = "<form onsubmit=\"press('D" + i + "');return false\"><input type=\"range\" min=\"1\" max=\"10\" step=\"1\" value=\"1\" id=\"button_duration_d" + i +"\" oninput=\"this.nextElementSibling.innerHTML = zeroBeforeSingleDigit(this.value)\"> <span>01</span> <br><input type=\"submit\" value=\"Einschalten\" label=\"duration\"></form>";
                        document.getElementById('button_duration_d' + i).value = zeroBeforeSingleDigit(json_config['D' + i].duration);
